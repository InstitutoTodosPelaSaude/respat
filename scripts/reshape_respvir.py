        ...
        if lab == 'DB Molecular':
            ...
            if 'resultado' not in dfL.columns.tolist():
                if 'resultado_norm' in dfL.columns.tolist():
                    ...
                    dfL['resultado'] = dfL['resultado'].apply(lambda x: 'NAO DETECTADO' if x == 'NEGATIVO' else 'DETECTADO')
                else:
                    if 'resultado_original' in dfL.columns.tolist():
                        dfL.rename(columns={'resultado_original':'resultado'}, inplace=True)
                        dfL['resultado'] = dfL['resultado'].apply(lambda x: 'NAO DETECTADO' if x == 'NDT' else 'DETECTADO')
                    else:
