...
if __name__ == '__main__':
    ...
    path = "/Users/Anderson/Library/CloudStorage/GoogleDrive-anderson.brito@itps.org.br/Outros computadores/My Mac mini/google_drive/ITpS/projetos_itps/resp_pathogens/analyses/fleury_dev/"
    ...
    pathogens = {'FLUA': ['Covidflursvgx - Influenza A', 'Virusmol, Influenza A', 'Virusmol, Influenza A/H1','Virusmol, Influenza A/H1-2009', 'Virusmol, Influenza A/H3', 'Vírus Influenza A (Sazonal)', 'Vírus respiratórios - Influenzavirus A'],
                 'FLUB': ['Covidflursvgx - Influenza B', 'Virusmol, Influenza B', 'Vírus respiratórios - Influenzavirus B'],
                 'VSR': ['Covidflursvgx - Vírus Sincicial Respiratório', 'Virusmol, Vírus Sincicial Respiratório', 'Vírus Sincial Respiratório', 'Vírus respiratório - Sincicial', 'Vírus respiratórios - Vírus Sincicial Respira'],
                 'SC2': ['Covid 19, Antígeno, teste rápido', 'Covid 19, Detecção por PCR', 'Covidflursvgx - SARS-CoV-2', 'Virusmol, SARS-CoV-2'], 'META': ['Virusmol, Metapneumovírus Humano'],
                 'PARA': ['Virusmol, Parainfluenza 1', 'Virusmol, Parainfluenza 2', 'Virusmol, Parainfluenza 3', 'Virusmol, Parainfluenza 4', 'Vírus respiratórios - Parainfluenzavirus 1', 'Vírus respiratórios - Parainfluenzavirus 3'],
                 'ADENO': ['Virusmol, Adenovírus', 'Vírus respiratórios - Adenovírus'],
                 'COVS': ['Virusmol, Coronavírus 229E', 'Virusmol, Coronavírus HKU1', 'Virusmol, Coronavírus NL63', 'Virusmol, Coronavírus OC43'],
                 'ENTERO': [],
                 'RINO': [],
                 'BOCA': [],
                 'BAC': ['Virusmol, Bordetella parapertussis', 'Virusmol, Bordetella pertussis', 'Virusmol, Chlamydophila pneumoniae']}
    ...
    def fix_datatable(dfL, pathogens):
        ...
        # dfN = dfL
        dfN = pd.DataFrame()  # create empty dataframe, and populate it with reformatted data from original lab dataframe
        id_columns = ['CODIGO REQUISICAO', 'IDADE', 'SEXO', 'DATA COLETA', 'MUNICIPIO', 'ESTADO']
        ...
        alltypes = []
        positives = ['DETECTADO (POSITIVO)', 'Influenza A - POSITIVO', 'Influenza B - POSITIVO', 'P O S I T I V O', 'POSITIVO']
        for i, (code, dfR) in enumerate(dfL.groupby('CODIGO REQUISICAO')):
            ...
            for p, t in pathogens.items():
                ...
                for g in t:
                    target_pathogen[g] = p
            # print(target_pathogen)

            # print('### ' + str(len(dfR['SEXO'].tolist())) + ' ###')
            exame = list(set(dfR['EXAME'].tolist()))[0] + ' = ' + str(len(dfR['SEXO'].tolist()))
            if exame not in alltypes:
                alltypes.append(exame)

            total_tests = len(dfR['EXAME'].tolist()) # number of total tests per exam

            # iterate over different test types
            if total_tests == 1: # single target
                # pass
                single_tests = {'2019NCOV': 'covid', 'AGCOVIDNS': 'covid', 'COVID19SALI': 'covid', 'COVID19GX': 'covid',
                                'INFLUENZAPCR': 'flu', 'AGSINCURG': 'vsr', 'VRSAG': 'vsr'}

                exam = list(set(dfR['EXAME'].tolist()))[0] # exam name
                # exam = 'AGRESPVIR' # REMOVER COMENTÁRIO
                if exam == 'AGRESPVIR':
                    pass
                else:
            ...
            elif total_tests == 4:  # panel 4
                # pass
                exam = list(set(dfR['EXAME'].tolist()))[0]
                if exam == 'AGRESPVIR':
                    pass
                else:
                    ...
                    for pathogen, dfG in dfR.groupby('pathogen'):
                        ...
                        # print(exame, pathogen, test_result, result)
                        # print('\n#\n')

            elif total_tests == 7: # panel 7
                # print(exame)
                # print(set(dfR['PATOGENO'].tolist()))
                pass

            elif total_tests == 20:  # panel 20
                # pass
                exam = list(set(dfR['EXAME'].tolist()))[0]
                dfR.insert(1, 'test_kit', 'test_20')
                dfR['pathogen'] = dfR['PATOGENO'].apply(lambda x: target_pathogen[x])
                ...
                if os.path.isdir(input_folder + element) == True:
                    ...
                    for filename in sorted(os.listdir(input_folder + element)):
                        if filename.split('.')[-1] in ['tsv', 'csv', 'xls', 'xlsx'] and filename[0] not in ['~', '_']:
                            ...
                            # print(df[['test_id', 'EXAME', 'FLUA_test_result', 'FLUB_test_result', 'VSR_test_result', 'SC2_test_result', 'META_test_result', 'RINO_test_result', 'PARA_test_result', 'ADENO_test_result', 'BOCA_test_result', 'COVS_test_result', 'ENTERO_test_result', 'BAC_test_result']])
    ...
    dfT['date_testing'] = pd.to_datetime(dfT['date_testing']) # , format='%Y-%m-%d', errors='ignore'
    print(set(dfT['date_testing'].tolist()))
    print(type(dfT['date_testing'].tolist()[0]))
    ...
    print(sorted(list(set(dfT['age'].tolist()))))
    ...
    key_cols = ['lab_id', 'test_id', 'test_kit', 'sample_id', 'state', 'location', 'sex', 'date_testing', 'epiweek',
