...
@asset(
    compute_kind="python", 
    deps=[get_asset_key_for_model([respiratorios_dbt_assets], "hpardini_final")]
)
def hpardini_remove_used_files(context):
    ...
    engine = create_engine(f'postgresql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}')
    used_files = pd.read_sql_query(f"SELECT DISTINCT file_name FROM respiratorios.{raw_data_table}", engine).file_name.to_list()
    engine.dispose()
