...
from .assets import SIVEP_FILES_FOLDER, SIVEP_FILES_EXTENSION
...
@sensor(
    job=sivep_all_assets_job,
    default_status=DefaultSensorStatus.RUNNING
)
def new_sivep_file_sensor(context: SensorEvaluationContext):
    """
    Check if there are new files in the sivep folder and run the job if there are.
    The job will only run if the last run is finished to avoid running multiple times.
    """
    # Check if there are new files in the sivep folder
    file_system = FileSystem(root_path=SIVEP_FILES_FOLDER)
    files = file_system.list_files_in_relative_path("")
    valid_files = [file for file in files if file.endswith(SIVEP_FILES_EXTENSION)]
    if len(valid_files) == 0:
        return

    # Get the last run status of the job
    job_to_look = 'sivep_all_assets_job'
    ...
    if last_run_status in FINISHED_STATUSES or last_run_status is None:
        ...
        yield SkipReason(f"There are files in the sivep folder, but the job {job_to_look} is still running with status {last_run_status}. Files: {valid_files}")

# Failure sensor that sends a message to slack
sivep_slack_failure_sensor = make_slack_on_run_failure_sensor(
    monitored_jobs=[sivep_all_assets_job],
